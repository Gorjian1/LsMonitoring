...
public partial class MainWindow : Window
{
    ...
    private void OnReadingsReady(int nodeId, NodeReadings nodeReadings)
    {
        ...
        TriggerTelegramAlarmIfNeeded(nodeId, latest);
    }

    private void TriggerTelegramAlarmIfNeeded(int nodeId, Reading? latest)
    {
        var service = _telegramAlertService;
        if (latest is null || service is null)
        {
            TelegramAlertService.LogDiagnostic(
                $"alarm-skip node={nodeId} latest={(latest is null ? "null" : "ok")} service={(service is null ? "null" : "ok")} telegram_enabled={_config.Telegram.Enabled}");
            return;
        }
        ...
        _ = SafeUpdateAlarmAsync(service, nodeId, "A", isACritical, aDeviation ?? 0, latest.Timestamp);
        _ = SafeUpdateAlarmAsync(service, nodeId, "B", isBCritical, bDeviation ?? 0, latest.Timestamp);
    }

    private static async Task SafeUpdateAlarmAsync(TelegramAlertService service, int nodeId, string axis, bool isCritical, double value, DateTime timestamp)
    {
        try
        {
    ...
    private void RefreshCurrentNode(Reading? previousLatest = null)
    {
        ...
        PlotMetaText.Text = $"{PlotPointCountLabel(visibleReadings.Count, readings.Count)} · {PlotWindowLabel()} · {ThresholdModeLabel()}{PlotCutoffLabel()}";
    ...
    private void ApplyPlotLimitStateToControls()
    {
        ...
        {
            LimitPlotButton.Content = "Ограничить";
        ...
        {
            PickPlotLimitButton.Content = _isPickingPlotCutoff ? "✓" : "🖌";
