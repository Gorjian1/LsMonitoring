...
Telegram и Push поддерживают обновление активной тревоги с длительностью. Почта и SMS по умолчанию отправляют старт тревоги; отправку окончания можно включить отдельно, если важнее полная история, чем экономия лимитов.
...
- пароль альтернативного SMTP сохраняется в защищённом base64-поле с DPAPI на Windows; креды встроенной сервисной почты в `config.json` не хранятся (вшиты в сборку из GitHub Secrets);
